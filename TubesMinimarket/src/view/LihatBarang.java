...
public class LihatBarang extends JFrame {
    ...
    public LihatBarang(int pilih) {

        ArrayList<Barang> users = new controller.Controller().getAllBarang();
        ...
        model.addColumn("Persen_Diskon");
        model.addColumn("Kategori");
        table = new JTable(model);

        //Looping Data to Table
        for (int i = 0; i < users.size(); i++) {
            Barang current = users.get(i);
            Object[] addBarang = new Object[7];
            addBarang[0] = current.getKodeBarang();
            addBarang[1] = current.getNamaBarang();
            addBarang[2] = current.getStokBarang();
            addBarang[3] = current.getHargaBarang();
            addBarang[4] = current.getKarduluasaBarang();
            addBarang[5] = current.getPersenDiskon();
            addBarang[6] = current.getKategori();
            model = (DefaultTableModel) table.getModel();
            model.addRow(addBarang);
        }
